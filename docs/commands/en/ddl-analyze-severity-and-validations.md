...
| `INDICE_COLUNA_INEXISTENTE` | `high` | Index references missing column | Index column not found | Index includes column absent from table. Ex.: index on `SALES(ISSUE_DATE)` where `ISSUE_DATE` is missing. |
